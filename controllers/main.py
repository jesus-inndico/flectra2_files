...
def ensure_db(redirect='/web/database/selector'):
    ...
    if db != request.session.db:
        ...
        request.session.db = db #here originally <- 1 tab
    
    print('*****Line 164 Request session db: *********', request.session.db)
...
def login_and_redirect(db, login, key, redirect_url='/web'):
    ...
    print('******L 311 login_and_redirect: session authenticate request *******')
...
class Home(http.Controller):
    ...
    def index(self, s_action=None, db=None, **kw):
        ...
        return werkzeug.utils.redirect('http://localhost:5173', 303)

    # Inndico
    def webclient_bootstrap_render(self, s_action=None, **kw):
        try:
    ...
    def web_login(self, redirect=None, **kw):
        ...
        print('*****line 512 request.uid******', request.uid)
        ...
        # Originally this was the render but we decided to render wbclient bootstrap
        # response = request.render('web.login', values)
        response = self.webclient_bootstrap_render() 
        response.headers['X-Frame-Options'] = 'DENY'
        session_info = request.env['ir.http'].session_info()
        session_id =  request.session.sid
        session_info['session_id'] = session_id
        
        print('*******Line 541 Resp Headers******:', response.headers)
        print('******Line 542 request.params**********:', request.params)
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        return env.get_template("we.template").render(d)
        # return request.render('web.template', qcontext=d)
    ...
    @http.route('/mopi_redirect', type='http', auth='none', website=True)
    def mi_metodo(self):
        session_info = http.request.env['ir.http'].sudo().search([])
        context = {
            'session_info': session_info,
        }
        return http.request.render('web.template', context)
...
class Session(http.Controller):
    ...
    @http.route('/web/session/get_session_info', type='http', auth="none")
    def get_session_info(self):
        request.session.check_security()
        request.uid = request.session.uid
        print('**** REQUEST UID MAIN ******:', request.uid)
        request.disable_db = False
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        res_auth = request.env['ir.http'].session_info()
        print('*******Session auth value ********:', res_auth)
        return res_auth

    @http.route('/web/session', type='http', auth='user', methods=['GET'])
    def session(self):
