...
class Home(http.Controller):
    ...
    def web_client(self, s_action=None, **kw):
        ...
        print('********Line 483 UID************: ', request.uid)
        try:
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            old_uid = request.uid
            print('*******Line 521 OLD UID*********:', old_uid)
            uid = request.session.authenticate(request.session.db, request.params['login'], request.params['password'])
            print('******Line 520 uid**********: ', uid)
            if uid is not False:
                request.params['login_success'] = True
        ...
        print('********Line 535 request.session.get********', request.session.get('auth_login'))
...
#TODO : Heredar vista database/selector. XPath -> Eliminar el content
...
class Session(http.Controller):
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        print('*******Session auth value ********:', request.env['ir.http'].session_info())
